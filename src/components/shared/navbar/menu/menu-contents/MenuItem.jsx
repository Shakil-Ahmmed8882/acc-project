...
const MenuItem = ({ path, label, isBrand, className}) => {
  ...
  return (
    <Link 
      ...
      className={`${className? `${className}`:'text-[#cacaca]'} mt-7 md:mt-0 relative z-40 flex gap-1 group text-[14px] hover:text-hover-txt-clr hover:smooth-transition before:content-[''] before:bg-hover-txt-clr before:w-0 before:h-1 before:my-8 md:before:mt-11 before:absolute before:transition-all before:duration-300 before:ease-linear hover:before:w-[100px] md:hover:before:w-full smooth-transition`} href={path}>
      {label}
