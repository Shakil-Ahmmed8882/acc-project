...
const MenuIcon = ({label}) => {
    ...
    return (
        <div className="flex-1">
            <button  className={`${label && 'flex gap-3 items-center justify-center'} !z-50`} onClick={() => setIsMenuOpen(!isMenuOpen)}>
                <svg className={`!z-50 ${isMenuOpen?'rotate-180':'rotate-0'} smooth-transition`} xmlns="http://www.w3.org/2000/svg" width="18" height="16" viewBox="0 0 18 16" fill="none">
                ...
                className="text-white uppercase text-[13px] md:text-[16px] font-riviera">{label} </span>
            </button>
        </div>
    );
};
